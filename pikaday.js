...
let hasEventListeners = !!window.addEventListener;
...
let removeEvent = function (el, e, callback, capture) {
  if (hasEventListeners) {
...
let hasClass = function (el, cn) {
  return (` ${el.className} `).indexOf(` ${cn} `) !== -1;
};

let addClass = function (el, cn) {
  if (!hasClass(el, cn)) {
    el.className = (el.className === '') ? cn : `${el.className} ${cn}`;
  }
};

let removeClass = function (el, cn) {
...
let setToStartOfDay = function (date) {
...
let fireEvent = function (el, eventName, data) {
  let ev;
...
let adjustCalendar = function (calendar) {
  if (calendar.month < 0) {
 ...
 */
let renderDayName = function (opts, day, abbr) {
  day += opts.firstDay;
  while (day >= 7) {
    day -= 7;
  }
...
let isoWeek = function (date) {
  ...
  let yearDay = date.getDate();
  let weekDay = date.getDay();
  let dayInFirstWeek = 4; // January 4th
  let dayShift = dayInFirstWeek - 1; // counting starts at 0
  let daysPerWeek = 7;
  let prevWeekDay = function (day) { return (day + daysPerWeek - 1) % daysPerWeek; };

  // Adjust to Thursday in week 1 and count number of weeks from date to week 1.

  date.setDate(yearDay + dayShift - prevWeekDay(weekDay));

  let jan4th = new Date(date.getFullYear(), 0, dayInFirstWeek);
  let msPerDay = 24 * 60 * 60 * 1000;
  let daysBetween = (date.getTime() - jan4th.getTime()) / msPerDay;
  let weekNum = 1 + Math.round((daysBetween - dayShift + prevWeekDay(jan4th.getDay())) / daysPerWeek);
  return weekNum;
};

let renderWeek = function (d, m, y) {
  let date = new Date(y, m, d);
  let week = hasMoment ? moment(date).isoWeek() : isoWeek(date);
  return `<td class="pika-week">${week}</td>`;
};

let renderRow = function (days, isRTL, pickWholeWeek, isRowSelected) {
  return `<tr class="pika-row${pickWholeWeek ? ' pick-whole-week' : ''}${isRowSelected ? ' is-selected' : ''}">${(isRTL ? days.reverse() : days).join('')}</tr>`;
};

let renderBody = function (rows) {
  return `<tbody>${rows.join('')}</tbody>`;
};

let renderHead = function (opts) {
  let i; let
    arr = [];
...
let renderTitle = function (instance, c, year, month, refYear, randId) {
  let i; let j; let arr;
  let opts = instance._o;
  let isMinYear = year === opts.minYear;
  let isMaxYear = year === opts.maxYear;
  let html = `<div id="${randId}" class="pika-title" role="heading" aria-live="assertive">`;
  let monthHtml;
  let yearHtml;
  let prev = true;
  let next = true;
...
let renderTable = function (opts, data, randId) {
 ...
 * Pikaday constructor
 */
const Pikaday = function (options) {
  let self = this;
...
Pikaday.prototype = {
  ...
  adjustDate(sign, days) {
    ...
    let difference = parseInt(days, 10) * 24 * 60 * 60 * 1000;
  ...
  render(year, month, randId) {
    ...
    let daysInPreviousMonth = getDaysInMonth(yearOfPreviousMonth, previousMonth);
    ...
    for (let i = 0, r = 0; i < cells; i++) {
      ...
      let isToday = compareDates(day, now);
      let hasEvent = opts.events.indexOf(day.toDateString()) !== -1;
